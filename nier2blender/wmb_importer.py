...
def construct_armature(name, bone_data_array):			# bone_data =[boneIndex, boneName, parentIndex, parentName, bone_pos, bone_rot, bone_number ]
    ...
    ob['bone_mapping'] = {}		# 添加骨骼索引数据k = 索引id，v = 骨骼名
...
def consturct_materials(texture_dir ,material):
    ...
    for texturesType in textures.keys():
        ...
        if flag:
            ...
            texture_file = "%s%s.dds" % (texture_dir, textures[texturesType].upper())

            # print('-----Load (material)%s Texture From %s%s.dds' % (material_name, texture_dir, textures[texturesType].upper()))
